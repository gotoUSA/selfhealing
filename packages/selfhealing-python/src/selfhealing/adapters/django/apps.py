        ...
        try:
            ...
            for domain in core_domains:
                try:
                    ...
                    logger.warning(f"[SelfHealing] Failed to bootstrap domain '{domain}': {e}")

            if created_domains:
                logger.info(f"[SelfHealing] Core domains bootstrapped: {created_domains}")

            if existing_domains:
                logger.debug(f"[SelfHealing] Core domains already existed: {existing_domains}")

        except ImportError:
            logger.debug("[SelfHealing] CircuitBreakerState model not available")
        except Exception as e:
            # Best-effort: 실패해도 시스템은 시작
            logger.warning(f"[SelfHealing] Failed to bootstrap core domains (non-fatal): {e}")
