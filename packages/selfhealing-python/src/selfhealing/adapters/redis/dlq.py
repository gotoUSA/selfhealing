...
class RedisDLQRepository(FailedOperationRepository):
    ...
    def get(self, entry_id: int) -> FailedOperationData | None:
        """
        Get DLQ entry by ID.
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        resolved_count = len(
            self.get_by_status(FailedOperationStatus.RESOLVED.value, limit=10000)
        )
        requires_review_count = len(
            self.get_by_status(FailedOperationStatus.REQUIRES_REVIEW.value, limit=10000)
        )
        rejected_count = len(
            self.get_by_status(FailedOperationStatus.REJECTED.value, limit=10000)
        )
        archived_count = len(
            self.get_by_status(FailedOperationStatus.ARCHIVED.value, limit=10000)
        )
