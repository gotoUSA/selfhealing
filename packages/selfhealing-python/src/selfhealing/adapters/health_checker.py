...
class TTLCacheStrategy(HealthCheckStrategy):
    ...
    def configure(self, check_callback: Callable[[str], bool], ttl: float = 5.0) -> None:
        """콜백 및 TTL 설정"""
        self._check_callback = check_callback
        self._ttl = ttl
...
class LinuxTCPInfoStrategy(HealthCheckStrategy):
    ...
    def check(self, target: str) -> bool:
        try:
            ...
            logger.debug(f"[LinuxTCPInfoStrategy] Health check failed for {target}: {e}")
            return False
...
class SimpleSocketStrategy(HealthCheckStrategy):
    ...
    def check(self, target: str) -> bool:
        try:
            ...
            logger.debug(f"[SimpleSocketStrategy] Health check failed for {target}: {e}")
            return False
...
class PortableHealthChecker:
    ...
    def _select_strategy(self, force_strategy: Optional[str]) -> HealthCheckStrategy:
        """환경에 맞는 최적 전략 선택"""
