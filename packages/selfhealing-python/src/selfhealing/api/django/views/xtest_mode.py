...
# =============================================================================
# Stage 51: Observability & Blast Radius Views
...
def _add_healing_event(event: Dict[str, Any]):
    """힐링 이벤트 기록."""
...
def _add_healing_incident(incident: Dict[str, Any]):
    """힐링 인시던트 기록."""
...
class HealingTimelineView(XTestModeMixin, APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            # 로컬 이벤트 추가
            with _healing_events_lock:
                local_events = list(_healing_events[-limit:])
            ...
            if service_filter:
                ...
                local_events = [
                    ...
                    if e.get("service") == service_filter
                ]
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            for _ in range(failure_count):
                cb_service.record_failure(
                    affected_service,
                    error_context={"source": "blast-radius-test"}
                )
            ...
            cb_service.force_close(
                affected_service,
                reason="Blast radius test cleanup",
                controlled_by="xtest"
            )
            ...
            return Response({
                "status": "error",
                "error": "blast_radius_test_failed",
                "message": str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class PostmortemGeneratorView(XTestModeMixin, APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            # 로컬 이벤트 추가
            with _healing_events_lock:
                local_events = list(_healing_events[-20:])
            ...
            postmortem = {
                ...
                "duration_seconds": None,  # 실제 계산은 복잡함
            ...
            # 인시던트 기록
            _add_healing_incident(postmortem)
            ...
            return Response({
                "status": "error",
                "error": "postmortem_generation_failed",
                "message": str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class RecordHealingEventView(XTestModeMixin, APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if request.data.get("include_snapshot", False):
            event["snapshot"] = _collect_system_snapshot()
        
        _add_healing_event(event)
        ...
        return Response({
            "status": "success",
            "event": event,
            "total_events": len(_healing_events),
            "timestamp": timezone.now().isoformat()
        })
...
class GetHealingIncidentsView(XTestModeMixin, APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        with _healing_events_lock:
            incidents = list(_healing_incidents[-limit:])
        
        return Response({
            "status": "success",
            "incidents": incidents,
            "total_count": len(_healing_incidents),
            "timestamp": timezone.now().isoformat()
        })
...
class MultiServiceBlastRadiusView(XTestModeMixin, APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        test_services = request.data.get(
            "test_services", 
            ["database", "payment", "external_api", "cache"]
        )
        failure_count = int(request.data.get("failure_count", 5))
        ...
        try:
            ...
            for affected_service in test_services:
                ...
                for _ in range(failure_count):
                    cb_service.record_failure(
                        affected_service,
                        error_context={"source": "multi-blast-radius-test"}
                    )
            ...
            return Response({
                "status": "error",
                "error": "multi_blast_radius_failed",
                "message": str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
