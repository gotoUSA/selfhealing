...
_health_checker: RedisHealthChecker | None = None
_local_limiter: LocalMemoryRateLimiter | None = None
...
class HybridRateLimitMiddleware:
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        ...
        if redis_healthy:
            # L2 (Redis) Rate Limit
            is_allowed, remaining, reset_time = self._check_redis_limit(request, rate_limit, window_seconds)
            mode = "normal"
        else:
            # L1 (Local Memory) Emergency Rate Limit
            is_allowed, remaining = self._check_local_limit(request, emergency_limit, emergency_window)
    ...
    def _check_redis_limit(
        ...
        try:
            ...
            # Log the fallback
            self._log_emergency_bypass(request, is_allowed, config["emergency_rate_limit"], reason=str(e))
