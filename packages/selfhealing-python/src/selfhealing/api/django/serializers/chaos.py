...
class StopConditionsConfigSerializer(serializers.Serializer):
    ...
    enabled = serializers.BooleanField(
        required=False, default=True, help_text="Stop Conditions 활성화 여부."
    )
...
class KillAllRequestSerializer(serializers.Serializer):
    ...
    reason = serializers.CharField(
        required=True, max_length=500, help_text="모든 실험 중단 이유"
    )
    operator = serializers.CharField(
        required=False, max_length=100, help_text="운영자 이메일 또는 ID"
    )
