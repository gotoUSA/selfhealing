...
class SyncSummarySerializer(serializers.Serializer):
    ...
    total_drifts_detected = serializers.IntegerField(help_text="감지된 총 Drift 수")
    total_drifts_corrected = serializers.IntegerField(help_text="보정된 총 Drift 수")
    max_drift_percent = serializers.FloatField(
        required=False, help_text="최대 Drift 퍼센트"
    )
...
class MetricSyncResponseSerializer(serializers.Serializer):
    ...
    synced_at = serializers.DateTimeField(help_text="동기화 시각 (ISO 8601)")
    actor = serializers.CharField(help_text="동기화 수행자 (사용자명)")
    dry_run = serializers.BooleanField(help_text="드라이런 모드 여부")
    results = serializers.DictField(
        child=DomainSyncResultSerializer(),
        help_text="도메인별 동기화 결과",
    )
    summary = SyncSummarySerializer(help_text="동기화 요약")
...
class MetricDriftItemSerializer(serializers.Serializer):
    ...
    in_memory = serializers.FloatField(help_text="현재 인메모리(Gauge) 값")
    actual = serializers.FloatField(help_text="DB에서 조회한 실제 값")
    drift = serializers.FloatField(help_text="차이 (actual - in_memory)")
    drift_percent = serializers.FloatField(required=False, help_text="Drift 퍼센트")
    is_critical = serializers.BooleanField(help_text="임계치 초과 여부")
...
class DriftReportResponseSerializer(serializers.Serializer):
    ...
    generated_at = serializers.DateTimeField(help_text="리포트 생성 시각 (ISO 8601)")
    metrics = DriftReportMetricsSerializer(help_text="메트릭별 Drift 정보")
    overall_health = serializers.ChoiceField(
        choices=["healthy", "warning", "critical", "incident"],
        help_text="전반적 상태",
    )
    max_drift_percent = serializers.FloatField(help_text="최대 Drift 퍼센트")
