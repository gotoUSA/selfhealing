...
class GlobalDailyAnchor:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GlobalDailyAnchor":
        ...
        cluster_anchors = [
            ClusterDailyAnchor.from_dict(ca)
            for ca in data.get("cluster_anchors", [])
        ]
        return cls(
            anchor_date=date.fromisoformat(data["date"]),
...
class CrossClusterAuditLinker:
    ...
    def create_local_anchor(self, target_date: Optional[date] = None) -> Optional[ClusterDailyAnchor]:
        """
        로컬 클러스터의 일일 앵커 생성.
        ...
        try:
            ...
            logger.info(
                f"[CrossClusterAuditLinker] Created local anchor for {target_date}: "
                f"{anchor.compute_anchor_hash()[:16]}..."
            )
            return anchor
    ...
    def get_local_anchor(self, target_date: date) -> Optional[ClusterDailyAnchor]:
        """
        로컬 앵커 조회.
        ...
        try:
            ...
            if existing:
                ...
                if any(ca.cluster_id == anchor.cluster_id for ca in cluster_anchors):
                    logger.info(
                        f"[CrossClusterAuditLinker] Anchor already submitted "
                        f"for {anchor.cluster_id}"
                    )
                    return True
    ...
    def get_global_anchor(self, target_date: date) -> Optional[GlobalDailyAnchor]:
        """
        글로벌 앵커 조회.
...
_linker: Optional[CrossClusterAuditLinker] = None
