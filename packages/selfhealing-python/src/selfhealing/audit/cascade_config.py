...
from selfhealing.settings import CascadeRetentionSettings, get_layered_settings
...
def get_cascade_chain_config() -> CascadeChainConfig:
    ...
    try:
        ...
        return CascadeChainConfig(
            max_chain_depth=getattr(settings, "SELFHEALING_CASCADE_MAX_DEPTH", 10),
            warn_at_depth=getattr(settings, "SELFHEALING_CASCADE_WARN_DEPTH", 7),
            block_on_exceed=getattr(
                settings, "SELFHEALING_CASCADE_BLOCK_ON_EXCEED", True
            ),
            detect_cycles=getattr(settings, "SELFHEALING_CASCADE_DETECT_CYCLES", True),
        )
    except Exception:
        # Django 없는 환경에서는 환경 변수 사용
        return CascadeChainConfig(
            max_chain_depth=int(os.environ.get("SELFHEALING_CASCADE_MAX_DEPTH", "10")),
            warn_at_depth=int(os.environ.get("SELFHEALING_CASCADE_WARN_DEPTH", "7")),
