...
class MetricReliabilityState:
    ...
    mode_transition_time: Optional[float] = None
    ...
    def is_data_fresh(self) -> bool:
        """데이터가 신선한지 여부."""
        return self.reliability_level in (ReliabilityLevel.HIGH, ReliabilityLevel.MEDIUM)
...
class MetricReliabilityManager:
    ...
    def __init__(
        ...
        safe_defaults_provider: Optional[Callable[[str], Any]] = None,
    ):
        """
    ...
    def _update_operating_mode(self, state: MetricReliabilityState) -> None:
        ...
        if state.reliability_level == ReliabilityLevel.UNKNOWN:
        ...
        elif state.reliability_level in (ReliabilityLevel.HIGH, ReliabilityLevel.MEDIUM):
            # 신선한 데이터
            if old_mode in (OperatingMode.STRICT, OperatingMode.EMERGENCY):
                ...
                # 안정화 기간 확인
                if (state.stabilization_start is not None and
                    time.time() - state.stabilization_start >= self._thresholds.stabilization_duration and
                    state.consecutive_successful_syncs >= self._thresholds.consecutive_syncs_for_normal):
                    # 안정화 완료 → 정상 모드
    ...
    def get_global_health(self) -> Dict[str, Any]:
        ...
        degraded = sum(1 for s in states.values() 
                      if s.reliability_level == ReliabilityLevel.LOW)
        unhealthy = sum(1 for s in states.values() 
    ...
    def force_strict_mode(self, domain: str, reason: str = "manual") -> None:
        ...
        with self._lock:
            ...
            logger.warning(
                f"[Reliability] {domain}: Forced STRICT mode ({reason})"
            )
...
_reliability_manager: Optional[MetricReliabilityManager] = None
