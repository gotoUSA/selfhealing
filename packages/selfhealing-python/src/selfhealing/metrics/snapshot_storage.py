...
class MetricSnapshotStorage:
    ...
    def load_all(self, max_age: Optional[float] = None) -> Dict[str, Dict[str, Any]]:
        """
        모든 값 로드.
    ...
    def get_snapshot_age(self) -> Optional[float]:
        """
        스냅샷 나이 조회.
...
_snapshot_storage: Optional[MetricSnapshotStorage] = None
_storage_lock = threading.Lock()


def get_snapshot_storage(storage_dir: Optional[str] = None) -> MetricSnapshotStorage:
    """
    싱글톤 스냅샷 저장소 인스턴스 반환.
