...
_current_domain: ContextVar[Optional[str]] = ContextVar(
...
def set_domain_context(domain: Optional[str]) -> Token:
    """
    도메인 컨텍스트 설정 (저수준 API).
...
class DomainMiddlewareMixin:
    ...
    def extract_domain_from_request(self, request) -> Optional[str]:
        """
        요청에서 도메인 추출.
