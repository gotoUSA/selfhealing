...
class TieredRedisProvider:
    ...
    def _get_global_client(self) -> Any:
        ...
        if self._global_client is None:
            ...
            if self._global_url == self._local_url:
                ...
                logger.debug(
                    "[TieredRedisProvider] Global Redis reusing local client"
                )
            else:
                import redis
    ...
    def health_check(self, scope: Optional[RedisScope] = None) -> dict:
        """
        Redis 상태 확인.
...
_provider: Optional[TieredRedisProvider] = None
