...
class RunbookRegistry:
    ...
    def _register_event_handlers(self) -> None:
        ...
        if bus:
            try:
                ...
                logger.warning("runbook_registry.event_subscribe_failed")
    ...
    def _restore_enabled_state(self) -> None:
        ...
        런북이 등록되기 전에 호출되므로, 나중에 등록되는 런북에 대한
        enabled 상태는 register() 내에서 별도로 처리한다.
        """
        ...
        if backend:
            try:
                ...
                logger.warning("runbook_registry.restore_state_failed")
    ...
    def _persist_enabled_state(self) -> None:
        ...
        if backend:
            try:
                ...
                logger.warning("runbook_registry.persist_state_failed")
    ...
    def _validate_all_step_params(self, runbook: Runbook) -> None:
        ...
        for step in runbook.steps:
            ...
            # 템플릿 변수({...}) 포함 값은 정적 검증에서 제외
            static_params = {k: v for k, v in step.params.items() if not (isinstance(v, str) and "{" in v and "}" in v)}
