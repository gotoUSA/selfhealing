...
class AdjustmentRecorder:
    ...
    def get_record(self, record_id: str) -> Optional[AdjustmentRecord]:
        """기록 조회"""
        with self._lock:
    ...
    def end_session(self, state: TuningState = TuningState.COMPLETED) -> Optional[TuningSession]:
        """현재 세션 종료"""
        with self._lock:
            ...
            logger.info(f"[AdjustmentRecorder] Ended session: {session.session_id if session else 'unknown'}")
            return session
    ...
    def get_current_session(self) -> Optional[TuningSession]:
        """현재 세션 조회"""
        with self._lock:
            if self._current_session_id:
    ...
    def _load_from_file(self):
        ...
        try:
            ...
            logger.info(f"[AdjustmentRecorder] Loaded {len(self._records)} records from file")
        except Exception as e:
