...
class AutoTuningService:
    ...
    def __init__(
        ...
        self._module_states: Dict[str, ModuleState] = {
            m: ModuleState.ENABLED for m in self.MODULES
        }
        ...
        self._disabled_by: Optional[str] = None
        self._disabled_reason: Optional[str] = None
        self._auto_enable_timer: Optional[Timer] = None
    ...
    def enable_module(
        ...
        with self._lock:
            if module not in self.MODULES:
                return {"error": f"Unknown module: {module}", "valid_modules": self.MODULES}
    ...
    def _get_last_module_adjustment(self, module: str) -> Optional[str]:
        """모듈의 마지막 조정 시간"""
        # 모듈-파라미터 매핑
        module_params = {
    ...
    def _record_audit_event(self, action: str, details: Dict[str, Any]) -> str:
        ...
        try:
            ...
            entry = AuditEntry(
                ...
                actor_type=details.get("enabled_by", details.get("disabled_by", details.get("updated_by", "system"))),
                actor_id=details.get("enabled_by", details.get("disabled_by", details.get("updated_by", "system"))),
            )
    ...
    def _disable_parameter_auto_tuning(self, parameter: str, until: Optional[datetime]):
        """특정 파라미터 자동 조정 비활성화"""
        # 파라미터-모듈 매핑
        param_module = {
