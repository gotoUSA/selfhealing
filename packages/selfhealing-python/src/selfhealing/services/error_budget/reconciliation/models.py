...
class ShadowBudget:
    ...
    accuracy_variance_percent: float | None = None
...
class ExcludedPeriod:
    ...
    # 연관 Fail-Safe 기간
    failsafe_period_id: str | None = None

    # 투명성 강화: 제외 당시 원본 데이터
    original_estimated_errors: int | None = None
    original_log_source: str | None = None
    original_adjustment_percent: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "duration_minutes": round(
                (self.ended_at - self.started_at).total_seconds() / 60, 2
            ),
            "reason": self.reason,
...
class ReconciliationConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ReconciliationConfig:
        return cls(
            ...
            max_adjustment_percent_per_cycle=data.get(
                "max_adjustment_percent_per_cycle", 10.0
            ),
            auto_exclude_short_periods=data.get("auto_exclude_short_periods", True),
            short_period_threshold_seconds=data.get(
                "short_period_threshold_seconds", 60.0
            ),
        )
