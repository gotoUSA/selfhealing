...
class EmergencyBackfillCalculator:
    ...
    def __init__(
        self,
        get_error_records: Optional[Callable[..., List[Any]]] = None,
        get_metrics_history: Optional[Callable[..., List[Dict[str, Any]]]] = None,
        ...
        if self._get_error_records:
            try:
                ...
                original_consumption = sum(
                    getattr(r, 'raw_duration_minutes', 1.0) 
                    for r in (records or [])
                )
            except Exception as e:
...
_backfill_calculator: Optional[EmergencyBackfillCalculator] = None
