...
class CheckOnUseMultiplierProvider:
    ...
    def __init__(
        ...
        domain_sensitivity: Optional[Dict[str, float]] = None,
        ...
        self._domain_sensitivity = domain_sensitivity or dict(DEFAULT_DOMAIN_SENSITIVITY)
    ...
    def _get_emergency_tracker(self) -> Any:
        ...
        if self._emergency_tracker is None:
            try:
                ...
                logger.warning(
                    "[Provider] NamespacedEmergencyTracker not available"
                )
    ...
    def get_all_multipliers(self) -> Dict[str, float]:
        ...
        return {
            level.name: value
            for level, value in self._multipliers.items()
        }
...
_check_on_use_provider: Optional[CheckOnUseMultiplierProvider] = None
