...
class ForensicRateLimiter:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            now = time.time()
            exception_ts = self._cleanup_old_timestamps(
                self._exception_timestamps, now
            )
            snapshot_ts = self._cleanup_old_timestamps(
                self._snapshot_timestamps, now
            )
            anomaly_ts = self._cleanup_old_timestamps(
                self._anomaly_timestamps, now
            )
            return {
...
_default_rate_limiter: Optional[ForensicRateLimiter] = None
...
class ForensicAuditBridge:
    ...
    def __init__(
        ...
        rate_limiter: Optional[ForensicRateLimiter] = None,
    ):
        """
