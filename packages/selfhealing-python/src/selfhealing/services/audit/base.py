...
def _get_trace_id_from_context(trace_id: str | None = None) -> str | None:
...
def _get_celery_context() -> dict | None:
    """Celery 컨텍스트 추출."""
    try:
...
def _try_flush_memory_buffer() -> int:
    ...
    try:
        ...
        def wal_write_entry(entry: dict) -> int | None:
            """버퍼 엔트리를 WAL에 기록."""
            try:
...
def get_wal_stats() -> dict[str, Any] | None:
    """WAL 통계 조회."""
    wal = _get_wal()
    if wal is None:
        return None
