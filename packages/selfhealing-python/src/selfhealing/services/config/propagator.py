    ...
    def propagate(self, change: GlobalConfigChange) -> bool:
        ...
        try:
            ...
            if change.scope == ConfigScope.GLOBAL:
                ...
                # LOCAL은 전파 불필요
                logger.debug(f"[GlobalConfigPropagator] LOCAL scope, skipping propagation")
                return True
    ...
    def _handle_message(self, data: str) -> None:
        ...
        try:
            ...
            if self._identity and change.source_cluster == self._identity.cluster_id:
                logger.debug(f"[GlobalConfigPropagator] Ignoring own message")
                return
...
_propagator: Optional[GlobalConfigPropagator] = None
