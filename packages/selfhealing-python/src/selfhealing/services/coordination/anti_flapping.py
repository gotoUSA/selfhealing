...
class AntiFlappingGuard:
    ...
    _last_recovery_at: Optional[datetime] = None
    """마지막 복구 완료 시각."""
    
    _flapping_lockout_until: Optional[datetime] = None
    """플래핑 잠금 해제 시각."""
    
    def check_transition_allowed(
        self,
        transition_history: Optional[List[datetime]] = None,
        ...
        if self._flapping_lockout_until and now < self._flapping_lockout_until:
            ...
            return (
                False,
                f"Flapping lockout active. {remaining:.0f}s remaining."
            )
    ...
    def check_cooldown_elapsed(
        ...
        return (True, f"Cooldown elapsed ({elapsed:.0f}s >= {self.level_cooldown_seconds}s)")
    ...
    def record_transition(self, at: Optional[datetime] = None) -> None:
        """
        레벨 전환 기록.
        ...
        self._transition_history = [
            t for t in self._transition_history if t > cutoff
        ]
    ...
    def record_recovery_complete(self, at: Optional[datetime] = None) -> None:
        """
        복구 완료 기록.
    ...
    def get_status(self) -> dict:
        ...
        one_hour_ago = now - timedelta(hours=1)
        recent_transitions = [
            t for t in self._transition_history if t > one_hour_ago
        ]
        ...
        return {
            ...
            "lockout_until": (
                ...
                if self._flapping_lockout_until else None
            ),
            "last_recovery_at": (
