...
class OptimisticLocalActionExecutor:
    ...
    def _schedule_central_sync(
        ...
        logger.debug(
            f"[OptimisticAction] Sync scheduled: id={result.action_id}"
        )
    ...
    def sync_pending(self) -> Dict[str, bool]:
        ...
        for action_id, result in pending:
            try:
                ...
                logger.debug(
                    f"[OptimisticAction] Sync completed: id={action_id}"
                )
            except Exception as e:
    ...
    def get_local_state(self, namespace: str) -> Optional[Dict[str, Any]]:
        """
        로컬 캐시된 상태 조회.
    ...
    def clear_local_cache(self, namespace: Optional[str] = None) -> None:
        """
        로컬 캐시 삭제.
...
_executor_instance: Optional[OptimisticLocalActionExecutor] = None
