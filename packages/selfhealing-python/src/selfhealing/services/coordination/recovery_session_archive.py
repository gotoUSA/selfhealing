...
class RecoverySessionArchiveData:
    ...
    def from_dict(cls, data: dict[str, Any]) -> RecoverySessionArchiveData:
        """딕셔너리에서 생성."""
        steps = [RecoveryStepArchiveData.from_dict(s) for s in data.get("steps", [])]
...
class RecoverySessionArchiveService:
    ...
    def get_session(self, session_id: str) -> RecoverySessionArchiveData | None:
        """
        세션 조회.
...
_archive_service: RecoverySessionArchiveService | None = None
