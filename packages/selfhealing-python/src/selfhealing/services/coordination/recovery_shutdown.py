...
class RecoveryShutdownStats:
    ...
    wait_ended_at: Optional[datetime] = None
    """대기 종료 시각."""
...
class RecoveryAwareShutdownHook:
    ...
    def __init__(
        ...
        on_force_shutdown: Optional[Callable[[], None]] = None,
    ):
        """
        Args:
    ...
    def on_force_shutdown(self, pending_requests: Optional[List[Any]] = None) -> None:
        """강제 종료 시 호출."""
        ...
        if self._on_force_shutdown:
            try:
                ...
                logger.error(f"[RecoveryAwareShutdownHook] Force shutdown callback error: {e}")
            ...
            if not self._check_recovery():
                ...
                if self._on_recovery_complete:
                    try:
                        ...
                        logger.error(f"[RecoveryAwareShutdownHook] Recovery complete callback error: {e}")
