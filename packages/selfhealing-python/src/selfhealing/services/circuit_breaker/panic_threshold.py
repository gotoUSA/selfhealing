...
class PanicThresholdMonitor:
    ...
    def __init__(
        self,
        config: Optional[PanicThresholdConfig] = None,
        circuit_breaker_service: Optional["CircuitBreakerService"] = None,
        ...
        self._last_check_result: Optional[PanicThresholdResult] = None
    ...
    def check_panic_threshold(self) -> PanicThresholdResult:
        ...
        if not self.config.enabled:
            return PanicThresholdResult(
                triggered=False,
                reason="Panic Threshold disabled"
            )
    ...
    def get_last_result(self) -> Optional[PanicThresholdResult]:
        """
        마지막 체크 결과 반환.
...
_monitor_instance: Optional[PanicThresholdMonitor] = None
