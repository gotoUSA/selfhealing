...
class DefaultEventCollector:
    ...
    def get_events(
        ...
        return [e for e in events if e.get("timestamp", now()) >= since]
...
class ResilienceValidator:
    ...
    def __init__(
        self,
        cb_provider: CircuitBreakerStatusProviderProtocol | None = None,
        event_collector: EventCollectorProtocol | None = None,
        emergency_provider: EmergencyModeProviderProtocol | None = None,
    ):
        """
        초기화.
    ...
    def validate(
        self,
        expectation: ResilienceExpectation | None,
    ...
    def _validate_retry(
        ...
        return (
            ...
            f"{actual_count} retries (expected: {expected_count})",
        )
    ...
    def _validate_graceful_degradation(
        ...
        fallback_passed, fallback_state = self._validate_fallback(
            assertion, experiment_start
        )
