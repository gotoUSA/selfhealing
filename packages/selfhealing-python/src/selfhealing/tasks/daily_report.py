...
class DailyAutonomousReport:
    ...
    def to_slack_message(self) -> str:
        ...
        total_operations = self.archived_count + self.expired_count + self.purged_count + self.recovered_count
...
class GenerateDailyAutonomousReportTask:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[GenerateDailyAutonomousReportTask] Report generated - " f"{len(report_data.entries)} entries")
            ...
            logger.error(f"[GenerateDailyAutonomousReportTask] Failed: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
            }
...
def generate_daily_autonomous_report(
    ...
    try:
        ...
        logger.info(f"[DailyReport] Report sent: {len(report.entries)} entries, " f"severity={severity}")
