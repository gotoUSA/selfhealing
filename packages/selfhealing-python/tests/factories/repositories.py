...
class InMemoryCircuitBreakerRepository:
    ...
    def get_or_create(self, service_name: str) -> MockCircuitBreakerStateData:
        ...
        if service_name not in self._states:
            self._states[service_name] = MockCircuitBreakerStateData(service_name=service_name)
...
class InMemoryDLQRepository:
    ...
    def list_pending(
        ...
        entries = [
            ...
            if e.status == DefaultValues.STATUS_PENDING and (domain is None or e.domain == domain)
        ]
