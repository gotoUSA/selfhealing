...
class TestGetAuditBufferFactory:
    ...
    def test_default_returns_memory_buffer(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=True):
            # SELFHEALING_BUFFER_TYPE 없으면 memory
            os.environ.pop("SELFHEALING_BUFFER_TYPE", None)

            from selfhealing.audit.resilience.buffer import (
                InMemoryAuditBuffer,
                get_audit_buffer,
            )
    ...
    def test_memory_type_returns_memory_buffer(self):
        """SELFHEALING_BUFFER_TYPE=memory는 InMemoryAuditBuffer 반환."""
        with mock.patch.dict(os.environ, {"SELFHEALING_BUFFER_TYPE": "memory"}):
            from selfhealing.audit.resilience.buffer import (
                InMemoryAuditBuffer,
                get_audit_buffer,
            )
    ...
    def test_disk_type_returns_disk_adapter(self, temp_db_path):
        """SELFHEALING_BUFFER_TYPE=disk는 DiskBufferAdapter 반환."""
        with mock.patch.dict(
            os.environ,
            {
                "SELFHEALING_BUFFER_TYPE": "disk",
                "SELFHEALING_DISK_BUFFER_DATA_DIR": temp_db_path,
                "SELFHEALING_DISK_BUFFER_LMDB_MAP_SIZE_MB": "100",
            },
        ):
            # 싱글톤 리셋
            from selfhealing.audit.persistence.config import reset_disk_buffer_settings
            from selfhealing.audit.persistence.disk_buffer import (
                DiskBufferAdapter,
                reset_disk_buffer,
            )
            from selfhealing.audit.resilience.buffer import get_audit_buffer
...
class TestFallbackChainDiskBuffer:
    ...
    def test_add_integrity_memory_uses_disk_buffer(self, temp_db_path):
        ...
        with mock.patch.dict(
            os.environ,
            {
                "SELFHEALING_DISK_BUFFER_DATA_DIR": temp_db_path,
                "SELFHEALING_DISK_BUFFER_LMDB_MAP_SIZE_MB": "100",
                "SELFHEALING_DISK_BUFFER_ENABLE_SHUTDOWN_HANDLERS": "false",
            },
        ):
            from selfhealing.audit.graceful_degradation.fallback import (
    ...
    def test_add_integrity_memory_fallback_to_memory(self):
        ...
        with mock.patch.dict("sys.modules", {"lmdb": None}):
            with mock.patch(
                "selfhealing.audit.persistence.disk_buffer.get_disk_buffer",
