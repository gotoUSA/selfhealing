...
class TestBatchWritePerformance:
    ...
    def test_batch_faster_than_individual(self, wal_dir):
        ...
        # 배치 쓰기 시간 측정
        wal_batch = WriteAheadLog(config=WALConfig(
            wal_dir=wal_dir + "_batch", sync_on_write=True
        ))
        start = time.time()
        ...
        # 개별 쓰기 시간 측정
        wal_single = WriteAheadLog(config=WALConfig(
            wal_dir=wal_dir + "_single", sync_on_write=True
        ))
        start = time.time()
