...
class TestCBNotificationHandlerActionableUrls:
    ...
    def test_notification_delegates_to_celery_with_service_name(self, mock_delay):
        """핸들러가 Celery task로 알림을 위임하고 service_name이 전달되는지 확인."""
        from selfhealing.services.circuit_breaker.actionable_alert_urls import (
            reset_actionable_alert_url_builder,
        )
        ...
        # Celery task에 service_name이 정확히 전달되었는지 확인
    ...
    def test_notification_delegates_without_env_vars(self, mock_delay):
        """환경변수가 없어도 Celery task 위임이 정상 수행되는지 확인."""
        from selfhealing.services.circuit_breaker.actionable_alert_urls import (
            reset_actionable_alert_url_builder,
        )
        from selfhealing.services.event_bus import (
...
class TestSlackBlockKitFormatter:
    ...
    def test_format_cb_slack_blocks_with_trace_url(self):
        ...
        trace_blocks = [
            ...
            if b["type"] == "section"
            and "text" in b
            and "mrkdwn" in str(b.get("text", {}).get("type", ""))
            and "jaeger" in str(b.get("text", {}).get("text", "")).lower()
        ]
    ...
    def test_admin_button_has_primary_style(self):
        ...
        admin_buttons = [e for e in action_blocks[0]["elements"] if e.get("action_id") == "view_admin"]
