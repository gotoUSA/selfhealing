...
class TestEnvironmentConfiguration:
    ...
    def test_custom_weights_from_env(self):
        ...
        with patch.dict(os.environ, {
            "SELFHEALING_EXCEPTION_WEIGHTS_JSON": json.dumps(custom_config)
        }):
            reset_exception_weight_map()
...
class TestDefaultWeights:
    ...
    def test_system_is_highest(self):
        """SYSTEM이 가장 높은 가중치."""
        assert DEFAULT_CATEGORY_WEIGHTS["SYSTEM"] >= max(
            v for k, v in DEFAULT_CATEGORY_WEIGHTS.items() if k != "SYSTEM"
        )
...
class TestIntegrationWithErrorCode:
    ...
    def test_all_known_error_codes_have_valid_weight(self):
        ...
        # 주요 ErrorCode 문자열 목록
        error_codes = [
            "SYSTEM_INTERNAL_ERROR", "SYSTEM_DATABASE_ERROR", "SYSTEM_DLQ_ERROR",
            "SERVICE_UNAVAILABLE", "SERVICE_CIRCUIT_OPEN", "SERVICE_TIMEOUT", "SERVICE_BAD_GATEWAY",
            "VALIDATION_FIELD_REQUIRED", "VALIDATION_FIELD_INVALID", "VALIDATION_PARSE_ERROR",
            "AUTH_NOT_AUTHENTICATED", "AUTH_TOKEN_INVALID", "AUTH_TOKEN_EXPIRED",
            "AUTHZ_PERMISSION_DENIED", "AUTHZ_GOVERNANCE_BLOCKED", "AUTHZ_ERROR_BUDGET_BLOCKED",
            "RESOURCE_NOT_FOUND", "RESOURCE_CONFLICT",
            "RATE_LIMIT_EXCEEDED", "RATE_THROTTLED",
            "CONFIG_LOCKED", "CONFIG_INVALID",
        ]
