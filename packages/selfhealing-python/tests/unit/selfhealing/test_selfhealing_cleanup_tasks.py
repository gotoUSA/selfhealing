...
class TestCleanupExpiredConfig:
    ...
    def test_run_success(self, mock_pending_config_service):
        ...
        mock_pending_config_service.cleanup_expired.assert_called_once_with(max_age_hours=24)
...
class TestCleanupTaskIntegration:
    ...
    def test_daily_cleanup_simulation(self, mock_runtime, mock_pending, mock_dlq):
        ...
        # 일일 요약 데이터 집계 가능 확인
        total_cleaned = archive_result["archived_count"] + cleanup_result["expired_count"] + expire_result["expired_count"]
