...
# =============================================================================
# 부분 실패 로깅 extra 메타데이터 동작 검증
# =============================================================================


class TestPartialFailureLoggingBehavior:
    """EmergencyCoordinator 부분 실패 로깅 동작 검증.

    액션 실행 중 실패 시 logger.warning에 extra 메타데이터가
    포함되는지, immediate 액션 fail-fast가 작동하는지 검증.
    """

    def test_partial_failure_logs_extra_metadata(self):
        """부분 실패 시 logger.warning에 extra 메타데이터가 포함되어야 한다."""
        coordinator = EmergencyCoordinator()

        actions = [
            CoordinationAction(type=ActionType.GOVERNANCE_STRICT, immediate=False),
            CoordinationAction(type=ActionType.CANARY_ROLLBACK, immediate=False),
        ]

        # 두 번째 액션만 실패시키기 위해 _execute_action_impl을 모킹
        original_impl = coordinator._execute_action_impl
        call_count = 0

        def failing_on_second(action, namespace, ttl):
            nonlocal call_count
            call_count += 1
            if call_count == 2:
                raise RuntimeError("canary rollback failed")
            return original_impl(action, namespace, ttl)

        coordinator._execute_action_impl = failing_on_second

        with patch("selfhealing.services.coordination.coordinator.logger") as mock_logger:
            result = coordinator.on_emergency_level_changed(
                old_level=EmergencyLevel.NORMAL,
                new_level=EmergencyLevel.LEVEL_3,
                namespace="global",
        ...
        # warning 호출에서 extra 파라미터 확인
        warning_calls = mock_logger.warning.call_args_list
        assert len(warning_calls) >= 1

        extra = warning_calls[0][1].get("extra", {})
        assert extra["alert_type"] == "partial_failure"
        assert extra["namespace"] == "global"
        assert isinstance(extra["succeeded_actions"], list)
        assert isinstance(extra["failed_actions"], list)
        assert ActionType.CANARY_ROLLBACK.value in extra["failed_actions"]

    def test_immediate_action_failure_aborts_remaining(self):
        """immediate 액션 실패 시 나머지 액션이 중단되어야 한다."""
        coordinator = EmergencyCoordinator()
        ...
        actions = [
            ...
            CoordinationAction(type=ActionType.BUDGET_MULTIPLIER, immediate=False),
        ]
        ...
        assert result.success is False
        # immediate 실패로 3번째 액션(BUDGET_MULTIPLIER) 스킵
        assert len(result.executed_actions) == 2

    def test_non_immediate_failure_continues(self):
        """non-immediate 액션 실패 시 나머지 액션이 계속 실행되어야 한다."""
        coordinator = EmergencyCoordinator()
        ...
        assert result.success is False
        # non-immediate이므로 3번째 액션도 실행됨
        assert len(result.executed_actions) == 3

    def test_partial_failure_extra_remaining_count(self):
        """부분 실패 시 remaining_count가 정확해야 한다."""
        coordinator = EmergencyCoordinator()
        ...
        extra = mock_logger.warning.call_args_list[0][1].get("extra", {})
        assert extra["remaining_count"] == 1
