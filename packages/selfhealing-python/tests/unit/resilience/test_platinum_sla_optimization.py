...
class TestDegradedModeHandler:
    ...
    def test_thread_safety(self):
        ...
        threads = [threading.Thread(target=reader) for _ in range(5)] + [threading.Thread(target=writer) for _ in range(5)]
...
class TestAdaptiveJitter:
    ...
    def test_relaxed_range_when_safe(self):
        ...
        jitter_range = AdaptiveJitter.get_jitter_range(
            error_budget_remaining=0.7, current_load=0.2  # 70% 남음 (> 50%)  # 20% 부하 (< 30%)
        )
    ...
    def test_stressed_range_when_budget_danger(self):
        ...
        jitter_range = AdaptiveJitter.get_jitter_range(error_budget_remaining=0.1, current_load=0.5)  # 10% 남음 (< 20%)

        assert jitter_range == AdaptiveJitter.JITTER_MIN_STRESSED

    def test_stressed_range_when_load_high(self):
        """부하 높을 때 STRESSED 범위"""
        jitter_range = AdaptiveJitter.get_jitter_range(error_budget_remaining=0.5, current_load=0.9)  # 90% 부하 (> 80%)
    ...
    def test_normal_range_for_mixed_state(self):
        """중간 상태에서 NORMAL 범위"""
        jitter_range = AdaptiveJitter.get_jitter_range(
            error_budget_remaining=0.4, current_load=0.5  # 40% (between 20% and 50%)  # 50% (between 30% and 80%)
        )
...
class TestPlatinumSLAIntegration:
    ...
    def test_adaptive_jitter_reduces_thundering_herd(self):
        ...
        # 부하가 높은 상황
        high_load_jitters = [AdaptiveJitter.calculate(current_load=0.9) for _ in range(100)]

        # 부하가 낮은 상황
        low_load_jitters = [AdaptiveJitter.calculate(error_budget_remaining=0.8, current_load=0.1) for _ in range(100)]
