...
class TestIsWithinBounds:
    ...
    def test_change_ratio_within_limit(self, safety_bounds):
        ...
        assert safety_bounds.is_within_bounds("timeout_ms", 1200, current_value=1000) is True

    def test_change_ratio_exceeds_limit(self, safety_bounds):
        """변경폭이 제한 초과인 경우 False."""
        # timeout_ms max_change: 0.3 (30%)
        # current: 1000, new: 1500 (50% 증가) -> NOT OK
        assert safety_bounds.is_within_bounds("timeout_ms", 1500, current_value=1000) is False
    ...
    def test_change_ratio_with_zero_current(self, safety_bounds):
        ...
        # 절대 범위만 검증
        assert safety_bounds.is_within_bounds("timeout_ms", 1000, current_value=0) is True
...
class TestUpdateBounds:
    ...
    def test_update_existing_bounds(self, safety_bounds):
        ...
        success = safety_bounds.update_bounds(
            "timeout_ms", {"min_value": 200, "max_value": 20000, "max_change_per_cycle": 0.2}
        )
    ...
    def test_add_new_bounds(self, safety_bounds):
        """새로운 한계 추가."""
        success = safety_bounds.update_bounds("new_param", {"min_value": 1, "max_value": 100, "max_change_per_cycle": 0.5})
    ...
    def test_update_invalid_bounds(self, safety_bounds):
        ...
        # min > max
        success = safety_bounds.update_bounds(
            "timeout_ms", {"min_value": 10000, "max_value": 100, "max_change_per_cycle": 0.3}
        )
    ...
    def test_update_with_defaults(self, safety_bounds):
        """부분 설정 시 기본값 사용."""
        success = safety_bounds.update_bounds("partial_param", {"min_value": 10})  # max_value와 max_change_per_cycle 누락
...
class TestResetToDefaults:
    ...
    def test_reset_to_defaults(self, safety_bounds):
        """기본값으로 리셋."""
        # 먼저 변경
        safety_bounds.update_bounds("timeout_ms", {"min_value": 999, "max_value": 999, "max_change_per_cycle": 0.99})
        safety_bounds.update_bounds("custom_param", {"min_value": 1, "max_value": 100, "max_change_per_cycle": 0.5})
            ...
            try:
                for i in range(50):
                    safety_bounds.update_bounds(
                        f"param_{param_id}_{i}", {"min_value": 1, "max_value": 100, "max_change_per_cycle": 0.3}
                    )
...
class TestEdgeCases:
    ...
    def test_very_small_change(self, safety_bounds):
        """매우 작은 변경."""
        # 0.1% 변경 - OK
        assert safety_bounds.is_within_bounds("timeout_ms", 1001, current_value=1000) is True

    def test_exactly_max_change(self, safety_bounds):
        """정확히 최대 변경폭."""
        # 정확히 30% 변경
        assert safety_bounds.is_within_bounds("timeout_ms", 1300, current_value=1000) is True

    def test_slightly_over_max_change(self, safety_bounds):
        """최대 변경폭 약간 초과."""
        # 30.1% 변경
        assert safety_bounds.is_within_bounds("timeout_ms", 1301, current_value=1000) is False
