...
class TestOrderCancelNegativeInputs:
    ...
    def test_cancel_delivered_order(self, client, auth_headers, user):
        ...
        # Assert - 5xx 에러 없음
        assert response.status_code < 500, (
            f"배송 완료 주문 취소에서 서버 에러!\n"
            f"status_code={response.status_code}"
        )

        # 400 예상 (취소 불가능한 상태)
        assert response.status_code == status.HTTP_400_BAD_REQUEST, (
            f"배송 완료 주문 취소가 허용됨: {response.status_code}"
        )
    ...
    def test_cancel_refunded_order(self, client, auth_headers, user):
        ...
        # 400 예상
        assert response.status_code == status.HTTP_400_BAD_REQUEST, (
            f"환불된 주문 취소가 허용됨: {response.status_code}"
        )
    ...
    def test_cancel_order_without_auth(self, client, user):
        ...
        # Assert - 5xx 에러 없음
        assert response.status_code < 500, (
            f"인증 없이 주문 취소에서 서버 에러!\n"
            f"status_code={response.status_code}"
        )

        # 401 예상
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, (
            f"인증 없이 주문 취소가 허용됨: {response.status_code}"
        )
