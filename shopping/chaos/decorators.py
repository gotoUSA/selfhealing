...
def inject_phase2_orphan_pg(payment_id: int = None, pg_response: dict = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP21_ORPHAN_PG,
        action="exception",
        context=context,
        level="error"
    )
    
    raise Phase2OrphanPGException(
        BREAKPOINT_BP21_ORPHAN_PG,
        f"[CHAOS BP-21] Internal failure after PG success. payment_id={payment_id}"
    )
...
def inject_phase2_rollback_failure(order_id: int = None, rollback_type: str = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP22_ROLLBACK_FAILURE,
        action="exception",
        context=context,
        level="error"
    )
    
    raise Phase2RollbackFailureException(
        BREAKPOINT_BP22_ROLLBACK_FAILURE,
        f"[CHAOS BP-22] Secondary rollback failure. order_id={order_id}"
    )
...
def inject_phase2_silent_task_failure(task_name: str = None, task_id: str = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP23_SILENT_TASK,
        action="silent_exception",
        context=context,
        level="error"
    )
    
    # This exception intentionally does NOT route to DLQ
    raise Phase2SilentTaskException(
        BREAKPOINT_BP23_SILENT_TASK,
        f"[CHAOS BP-23] Silent task failure (no DLQ). task={task_name}"
    )
...
def inject_phase2_point_orphan(user_id: int = None, order_id: int = None, points: int = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP29_POINT_ORPHAN,
        action="exception",
        context=context,
        level="error"
    )
    
    raise Phase2PointOrphanException(
        BREAKPOINT_BP29_POINT_ORPHAN,
        f"[CHAOS BP-29] Point accumulation failed. user={user_id}, points={points}"
    )
...
def inject_phase2_race_delay(payment_id: int = None, operation: str = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP27_RACE_AMPLIFICATION,
        action="delay",
        context=context,
        level="warning"
    )
...
def inject_phase2_cache_skip_invalidation(cache_key: str = None):
    ...
    chaos_config.log_chaos_event(
        breakpoint_name=BREAKPOINT_BP30_CACHE_DIVERGENCE,
        action="skip_invalidation",
        context=context,
        level="warning"
    )
