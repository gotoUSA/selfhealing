...
class InjectFailureMetadataSerializer(serializers.Serializer):
    ...
    failure_rate = serializers.FloatField(
        ...
        min_value=0.0,
        max_value=1.0,
        help_text="Failure rate between 0.0 and 1.0"
    )
    simulate_latency_ms = serializers.IntegerField(
        required=False,
        min_value=0,
        max_value=30000,
        help_text="Simulated latency in milliseconds"
    )
...
class ControlResponseSerializer(serializers.Serializer):
    ...
    action_applied = serializers.CharField(
        help_text="The action that was applied"
    )
