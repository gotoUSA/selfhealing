...
**이 레포 이름이 `selfhealing`인 이유**: 결제 흐름을 만들다가 "결제가 실패하면 이 주문은 어떻게 되나"를 파기 시작했고, 그 답(서킷 브레이커 · 재시도 · 실패 요청 보관/재실행)을 이 레포 안에서 직접 구현했다. 2025년 12월 그 계층을 독립 패키지로 떼어 냈고(따로따로 있던 것들을 합치면 제품이 되겠다고 봤다), 2026년 3월 별도 레포로 옮긴 뒤 [baldurhq/baldur](https://github.com/baldurhq/baldur)가 됐다. 이 레포에는 쇼핑몰과, 라이브러리가 되기 전의 결제 복구 코드(`shopping/services/payment_recovery_service.py`, 첫 커밋 2025-12-08)가 남아 있다.
...
- **관리자용 주문 목록 API는 주문 전체를 집계한 뒤 한 페이지를 자른다**: `OrderViewSet`이 항목 수를 `annotate(item_count=Count("order_items"))`로 붙여서, 관리자가 `GET /api/orders/`를 부르면 LEFT JOIN과 GROUP BY가 LIMIT보다 먼저 돈다. 합성 데이터(주문 20만·항목 40만)에서 첫 페이지 1초. 항목 수를 상관 서브쿼리로 바꾸고 `created_at` 인덱스를 두면 0.3ms(같은 데이터에서 실험으로 확인). 이 목록을 부르는 화면은 없고(관리 화면은 Django admin) 고객 목록은 자기 주문만 읽어서 그대로 뒀다.
- **Redis가 멈추면 캐시를 쓰지 않는 API까지 500을 돌려주고, 헬스 체크는 200이다**: 운영 설정은 전역 속도 제한(익명·사용자)을 켜 두는데 그 카운터가 Django 캐시(Redis)에 있어서 모든 API 요청이 Redis를 읽는다. Redis 컨테이너를 멈추고 재 보니 상품 목록·주문 목록·로그인·웹훅이 전부 500이었고(django_redis는 연결 오류를 그대로 올린다), `/health/`는 Redis를 확인하지 않아 200이었다. 캐시 오류를 무시하게 바꾸면 장애 동안 로그인 제한이 풀리므로 어느 쪽을 택할지는 정책 문제다. 적어도 헬스 체크는 Redis를 확인해야 한다. 개발용 `.env`는 캐시와 Celery 브로커를 같은 DB 0에 두므로 `cache.clear()`는 대기 중인 태스크까지 지운다.
- **카테고리 트리 캐시는 최대 1시간 동안 옛 값을 보여 줄 수 있다**: `/api/categories/tree/`는 결과를 1시간 캐시하고 카테고리·상품 저장 시그널로 지운다. 옛 트리가 남는 경우가 셋이다. 시그널이 트랜잭션 안에서 불려 커밋 전에 키를 지우는 경우, 커밋 전에 옛 데이터를 읽은 요청이 삭제 뒤에 저장하는 경우(삭제를 `on_commit`으로 옮겨도 남는다), `queryset.update()`가 시그널을 건너뛰는 경우다. 셋 다 재현했다(앞의 둘이 실제 편집에서 날 수 있는 틈은 1ms 남짓이다). 캐시가 아끼는 건 12ms 쿼리 하나라 TTL을 짧게 잡는 게 가장 싸고, 정확해야 하면 버전 키를 쓴다. 테스트 설정은 캐시를 DummyCache로 바꾸므로 CI는 이 동작을 보지 않는다. 트리 값 자체에도 두 가지가 있다. 부모 카테고리를 비활성으로 하면 그 자식들이 최상위로 올라오고, `product_count`는 하위 카테고리의 상품을 세지 않는다(그 카테고리의 상품 목록은 하위까지 보여 준다).
...
- **한 번의 머지가 코드를 떨어뜨렸던 일**: 2025-12-14, 쇼핑몰 브랜치와 결제 복구 브랜치를 합치면서 한쪽의 테스트와 다른 쪽의 코드가 섞여 들어갔다(취소 시 포인트 환불, 회원 등급 스냅샷, 락 순서 고정, 주문 실패 사유가 사라짐). 이 레포의 CI는 만든 날부터 테스트를 끝까지 돈 적이 없어서(2026년 3월부터는 사설 의존성 때문에 설치 단계에서 멈춤) 9개월 뒤에야 발견했고, 사라진 코드는 git에서 되살렸다. 두 갈래를 오래 벌려 두지 말 것, 그리고 CI가 빨간 채로 두지 말 것 — 이 레포에서 배운 가장 비싼 교훈이다.
