...
| 주문 취소: 입구 네 개, 규칙 하나 | `shopping/services/order_service.py` `cancel_by_customer` · `cancel_order`, `payment_service.py` `cancel_payment` | 취소는 주문 취소 버튼·결제 취소 버튼·토스 취소 웹훅·미결제 만료 배치 네 곳에서 들어온다. 주문 행 락을 잡은 뒤 상태를 검사해 다섯 번 동시에 눌러도 한 번만 취소된다(락 한 줄만 빼면 다섯 번 다 성공해 재고가 10개 늘어나는 걸 실험으로 확인). 결제 완료 주문은 주문 취소 버튼으로 눌러도 결제 취소(토스 환불)로 간다. 결제 전 주문은 결제가 살아 있으면(승인 중·입금 대기) 취소하지 않고, 아니면 결제를 먼저 닫아 뒤이은 승인을 막는다 — 만료 배치와 같은 울타리. 결제 마감은 취소된 주문을 결제 완료로 되살리지 않고, 결제 취소는 되돌릴 수 없는 토스 환불 전에 검증을 끝낸다. 예전엔 이 연결들이 빠져서 결제 완료 주문이 환불 없이 취소되거나, 승인 중 취소한 주문이 결제 완료로 되살아났다 (`shopping/tests/api/order/test_order_cancel_paths.py`) |
...
- **가상계좌 입금 대기 주문은 고객이 취소할 수 없다**: 입금 대기 중에 주문만 취소하면 뒤늦은 입금이 취소된 주문에 들어온다. 그래서 결제가 살아 있는 주문은 취소를 거절하고, 입금 기한이 지나면 토스 만료 웹훅으로 취소된다. 발급된 가상계좌를 토스 API로 먼저 닫고 취소하는 흐름은 만들지 않았다.
- **울타리 밖 경합으로 취소된 주문에 승인이 오면 사람이 환불한다**: 주문 취소가 승인 중인 결제를 막으므로 정상 경로로는 생기지 않지만, 그래도 오면 결제 마감은 주문을 되살리지 않고 결제를 `done`으로 남긴 뒤 critical 알림을 보낸다. 환불은 결제 취소 API로 한다(주문이 이미 취소돼 있으면 돈만 돌려주고 재고·포인트는 건드리지 않는다). 자동 환불은 도달할 경로가 없어서 만들지 않았다.
