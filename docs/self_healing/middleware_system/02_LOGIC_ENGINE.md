...
│  │                                                                            │  │
│  │  ┌─ 체크 유틸리티 ──────────────────────────────────────────────────────┐  │  │
│  │  │  GovernanceCheckMixin, @require_system_enabled, @require_governance  │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
│                            │                                                     │
│                            ▼                                                     │
│  ┌──────────────────────────────────────────────────────────────────────────┐  │
│  │            Interfaces Layer (추상 계약 - ABC 클래스)                      │  │
│  │                                                                            │  │
│  │  "무엇을 해야 하는지"만 정의. 실제 구현은 03_INFRA_ADAPTER.md 참조        │  │
│  │                                                                            │  │
│  │  ┌─ 저장소 인터페이스 ──────────────────────────────────────────────────┐  │  │
│  │  │  FailedOperationRepository, CircuitBreakerStateRepository,           │  │  │
│  │  │  SecurityIncidentRepository, StatisticsRepositoryInterface           │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌─ 인프라 인터페이스 ──────────────────────────────────────────────────┐  │  │
│  │  │  CacheProviderInterface, TaskQueueInterface, WebFrameworkInterface,  │  │  │
│  │  │  ConfigProviderInterface, RateLimitStorageInterface                  │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌─ 출력 인터페이스 ────────────────────────────────────────────────────┐  │  │
│  │  │  AuditLogAdapter (ABC), AlertAdapter (ABC), NotificationAdapter (ABC)│  │  │
│  │  │  ※ 이름은 Adapter지만 실제로는 인터페이스 (추상 클래스)               │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
│                            │                                                     │
│                            ▼                                                     │
...
│  │  인터페이스 ↔ 구현체 연결 관리. 런타임에 구현체 교체 가능                  │  │
│  │                                                                            │  │
│  │  - register_cache / get_cache                                             │  │
│  │  - register_queue / get_queue                                             │  │
│  │  - register_*_repo / get_*_repo                                           │  │
│  │  - register_audit_adapter / get_audit_adapter                             │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
                      03_INFRA_ADAPTER.md (구현체: Redis, Django, Celery 등)
```
...
| 레이어 | 역할 | 예시 |
|--------|------|------|
| **핵심 서비스** | 비즈니스 로직 실행 | CircuitBreaker, DLQ, Replay, Retry |
| **거버넌스** | 실행 가능 여부 판단 | Kill Switch (수동), ErrorBudget (자동) |
| **Interfaces** | 외부 의존성 추상화 (계약 정의) | `CacheProviderInterface` (ABC) |
| **Provider Registry** | 인터페이스 ↔ 구현체 연결 | `get_cache("redis")` → `RedisCacheAdapter` |
