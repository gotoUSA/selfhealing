...
def _check_shadow_evaluation(
    ...
    # === 진행 중 체크: PENDING/RUNNING 상태면 차단 ===
...
def _config_hash(config: dict) -> str:
    ...
    return hashlib.sha256(
        json.dumps(_sanitize(config)).encode()
    ).hexdigest()[:16]
```

**설계 결정 — `_sanitize()` 방어 로직 이유**:
`dict[str, Any]` 타입은 datetime, Decimal, Enum 등 비-JSON 타입이 섞일 수 있다.
`json.dumps(sort_keys=True)`만으로는 `TypeError`가 발생하므로,
순수 JSON 타입(int, str, bool, list, dict, None)으로 재귀적 정제 후 해시한다.
...
**설계 결정 — RUNNING 상태 차단 이유**:
Fail-open은 "Shadow 시스템 자체의 장애"에만 적용된다.
"평가가 아직 끝나지 않음"은 장애가 아니라 정상적인 상태이다.
운영자가 `evaluate_for_rollout()`을 호출한 것은 "안전성을 확인하겠다"는 의도이므로,
결과가 나오기 전에 무시하고 진행하면 Shadow Gate의 존재 의미가 없어진다.
차단 시 `block_message`에 `evaluation_id`와 `status`를 포함하여
운영자가 별도로 상태를 조회할 수 있게 안내한다.

**설계 결정 — TTL(시간 만료) 검증 이유**:
월요일에 평가를 돌려두고(Passed), 금요일에 `start_rollout()`을 누르면
4일 전 과거 데이터로 통과하는 문제가 생긴다.
`evaluation.completed_at` 필드(`models.py:65`)를 활용하여
`evaluation_ttl_hours` 설정값(기본 1시간) 이내의 결과만 유효하게 처리한다.
만료 시에는 차단 + "re-run evaluate_for_rollout()" 안내 로그를 남긴다.

**설계 결정 — Config Hash 대조 이유**:
평가 시점의 `candidate_config`(예: `threshold=10`)와 롤아웃 직전의
`candidate_config`(예: `threshold=15`로 수정)가 다를 수 있다.
`_config_hash()`로 결정론적 해시를 비교하여 불일치 시 재평가를 강제한다.
이는 Netflix Kayenta의 "분석 스냅샷 vs 배포 설정 일치 검증" 패턴과 동일하다.

**설계 결정 — 신뢰도(Confidence) 미달 시 경고만 발행하는 이유**:
`passed=True`는 시뮬레이션 로직이 "안전하다"고 판단한 것이고,
낮은 `confidence_score`는 "데이터가 부족하다"는 뜻이지 "위험하다"는 뜻이 아니다.
이벤트가 적은 새 서비스에서 설정 변경이 필요할 때 차단하면 운영이 마비된다.
단, Audit에 `shadow_evaluation_low_confidence` 액션으로 기록하여 추적 가능하게 한다.
향후 `block_on_low_confidence=True` 옵션으로 엄격 모드 전환을 지원한다.
...
**설계 결정 — `check_all_governance()`에 Shadow Evaluation을 추가하지 않는 이유**:

Shadow Evaluation은 `start_rollout()` 내부의 전용 로직(`_check_shadow_evaluation()`)으로만 존재한다.
범용 `check_all_governance()`에는 추가하지 않는다.

1. **의미론적 차이**: `check_all_governance()`의 기존 체크(Kill Switch, Emergency, Error Budget)는
   "시스템 전체 건강 상태"이고 `promote()` 때마다 재확인해야 한다.
   Shadow Evaluation은 "특정 설정 변경의 일회성 시뮬레이션 결과"이므로 성격이 다르다.

2. **promote() 시점 불필요**: `start_rollout()`으로 첫 클러스터에 적용된 후에는
   실제 Canary 메트릭이 존재한다. 시뮬레이션보다 실제 데이터가 우선한다.
   promote() 시점의 실시간 평가는 [301_LIVE_CANARY_EVALUATOR.md](301_LIVE_CANARY_EVALUATOR.md)에서 다룬다.

3. **시그니처 오염 방지**: `check_all_governance()`에 `check_shadow_evaluation`,
   `shadow_rollout_id` 파라미터를 추가하면, ReplayService/CB ManualControl 등
   기존 호출부가 불필요한 파라미터에 노출된다. 관심사 분리를 유지한다.
...
## 9. Confidence Graduation — 저신뢰도 시 데이터 수집 전략

### 9.1 개요

Shadow Evaluation 결과가 `passed=True`이지만 `confidence_score < min_confidence`인 경우,
데이터 부족으로 판단의 확실성이 낮다. 이때 단순 경고 후 통과시키는 것에 더해,
**백그라운드에서 추가 데이터를 수집하여 재평가할 수 있는 확장 포인트**를 마련한다.
...
[Low Confidence 감지]
     │
     ▼
 경고 로그 + Audit 기록 → start_rollout() 통과 (현재 동작)
     │
     ▼ (graduation_enabled=True일 때)
 Celery Beat 스케줄 등록: 주기적 이벤트 수 모니터링
     │
     ▼
 EventJournal 이벤트 수 ≥ target_events?
     │
     ├─ No → 계속 수집 (주기적 상태 리포트)
     │
     └─ Yes → 동일 설정으로 자동 재평가 (Celery task)
              │
              ├─ High Confidence 달성 → 운영자 알림:
              │   "충분한 데이터 확보. 엄격 모드 전환을 권장합니다."
              │   운영자가 수동으로 block_on_low_confidence=True 전환
              │
              └─ Low Confidence 유지 → 경고 리포트 + 분석 필요 안내
```

**핵심 원칙**: 시스템이 데이터를 모으고 판단을 제안하되,
정책 변경(경고→차단)의 최종 결정은 운영자가 수동으로 한다.
자동으로 엄격 모드를 활성화하지 않는다.

### 9.3 Settings

```python
# settings/config_shadow.py에 추가 (향후 구현)
...
Confidence Graduation의 핵심 로직(Celery Beat 스케줄, 이벤트 모니터링, 알림 연동)은
본 문서(300)의 스코프 밖이다. 300에서는 다음만 포함한다:

1. `_check_shadow_evaluation()`의 low confidence 경고 + Audit 기록 (4.3절에 반영 완료)
2. `block_on_low_confidence` 설정 (7.1절에 반영 완료)
3. Settings에 graduation 관련 필드 정의 (향후 구현 대비)
...
운영자                    Canary Service              Shadow Evaluator
  ...
  │                           │   ├─ TTL 만료? → 차단      │
  │                           │   ├─ Config Hash 불일치? → 차단
  │                           │   ├─ passed + low confidence → 경고 + 통과
  │                           │   ├─ passed → 통과         │
  │                           │   ├─ failed + bypass → Audit + 통과
  │                           │   └─ failed → 차단         │
  │                           ├─ 클러스터 적용             │
  │◀── 결과 ──────────────────│                            │
```
...
| 파일 | 변경 유형 | 내용 |
|------|-----------|------|
| `services/governance/checks.py` | 수정 | `BlockReason.SHADOW_EVALUATION_FAILED` 추가, `blocked_by_shadow_evaluation()` 팩토리 |
| `services/canary/service.py` | 수정 | `start_rollout()` 시그니처에 `bypass_shadow` 추가, `_check_shadow_evaluation()` 메서드 추가, `_config_hash()` 유틸리티 |
| `services/config_shadow/service.py` | 수정 | `evaluate_for_rollout()`, `get_latest_for_rollout()` 메서드 추가 |
| `settings/config_shadow.py` | 수정 | `evaluation_ttl_hours`, `block_on_low_confidence`, graduation 설정 추가 |
...
| 문서 | 관계 |
|------|------|
| [299_CONFIG_SHADOW_EVALUATOR.md](299_CONFIG_SHADOW_EVALUATOR.md) | 시뮬레이션 엔진 (본 문서의 의존 대상) |
| [301_LIVE_CANARY_EVALUATOR.md](301_LIVE_CANARY_EVALUATOR.md) | promote() 시점의 실시간 메트릭 평가 (본 문서의 후속) |
