#!/usr/bin/env python3
"""
Visibility-Focused Self-Healing Verification - User Point Lock Holder

CHOKE POINT: User.points 필드 (shopping_users.points)

이 스크립트는 모든 성공적인 결제 플로우가 반드시 통과해야 하는
User.points 필드에 대해 외부 락을 반복적으로 생성합니다.

왜 User.points가 최적의 choke point인가?
1. 모든 결제 완료 시 포인트 적립 (PointService.add_points)
2. 포인트 사용 시 SELECT FOR UPDATE로 락 획득 (PointService.use_points)
3. 비즈니스 검증(장바구니 비어있음, 재고 부족 등) 이후에 접근
4. 조기 실패로 우회 불가능

동작:
- N회 반복 (N ≥ 10)
- 각 반복: 락 획득 → 8-10초 유지 → ROLLBACK → 1-2초 대기
- Locust 부하 테스트와 동시 실행 시 락 경합 발생

환경변수:
    PGHOST, PGPORT, PGDATABASE, PGUSER, PGPASSWORD
    TARGET_USER_ID     - 락을 잡을 사용자 ID (0 = 자동 탐색)
...
class LockHolderConfig:
    ...
    def __init__(self):
        ...
        self.user_table = 'shopping_users'
        self.target_user_id = int(os.environ.get('TARGET_USER_ID', '0'))
...
class UserPointLockHolder:
    """User.points 필드에 대한 반복적 락 홀더"""
    ...
    def discover_target_user(self, cursor) -> Tuple[int, str]:
        """
        락을 잡을 대상 사용자 탐색
        
        조건:
        1. 포인트가 있는 활성 사용자 (로드 테스트 사용자 우선)
        2. 최근 활동이 있는 사용자
        """
        if self.config.target_user_id > 0:
            cursor.execute(
                f"SELECT id, username, points FROM {self.config.user_table} WHERE id = %s",
            ...
            if row:
                ...
                logger.error(f"Target user ID {self.config.target_user_id} not found!")
                sys.exit(1)
        
        # 자동 탐색: 로드테스트 사용자 중 포인트가 있는 사용자
        cursor.execute(f"""
            SELECT id, username, points 
            FROM {self.config.user_table} 
            WHERE username LIKE 'loadtest_user%%'
              AND is_active = TRUE
              AND points > 0
        ...
        if row:
            logger.info(f"AUTO_DISCOVER: Found load test user with points - id={row[0]}, username={row[1]}, points={row[2]}")
            return row[0], row[1]
        ...
        if row:
            logger.info(f"AUTO_DISCOVER: Found user with points - id={row[0]}, username={row[1]}, points={row[2]}")
            return row[0], row[1]
        ...
        if row:
            logger.info(f"AUTO_DISCOVER: Using first active user - id={row[0]}, username={row[1]}")
            return row[0], row[1]
        
        logger.error("AUTO_DISCOVER: No active users found!")
        sys.exit(1)
    ...
    def hold_lock_single_iteration(self, iteration: int) -> LockWindow:
        ...
        try:
            ...
            if self.target_user_id is None:
                self.target_user_id, self.target_username = self.discover_target_user(cursor)
                conn.rollback()  # 탐색 트랜잭션 종료
            
            # SELECT FOR UPDATE로 락 획득
            logger.info(f"[Iteration {iteration}/{self.config.lock_iterations}] Acquiring lock on user id={self.target_user_id}...")
            ...
            lock_query = f"""
                ...
                FOR UPDATE
            """
            cursor.execute(lock_query, (self.target_user_id,))
            row = cursor.fetchone()
            
            window.started_at = datetime.now()
            
            if not row:
                window.error = f"User id={self.target_user_id} not found"
            ...
            logger.info(f"   User: id={row[0]}, username={row[1]}, points={row[2]}")
    ...
    def run(self) -> list[LockWindow]:
        ...
        logger.info("CHOKE POINT: User.points (shopping_users.points)")
        logger.info("=" * 70)
...
def main():
    ...
    holder = UserPointLockHolder(config)
