...
def _maybe_snapshot_interval():
    ...
    if now - _interval_stats["last_snapshot"] >= 300:  # 5분
        ...
        avg_response = (
            ...
            if _interval_stats["response_times"] else 0
        )
        ...
        _soak_stats["intervals"].append({
            ...
            "error_rate": (
                _interval_stats["errors"] / _interval_stats["requests"] * 100
                if _interval_stats["requests"] > 0 else 0
            ),
        })
...
def on_test_stop(environment, **kwargs):
    ...
    # 최종 판정
    error_rate = (
        _soak_stats['total_errors'] / _soak_stats['total_requests'] * 100
        if _soak_stats['total_requests'] > 0 else 0
    )
    
    if error_rate < 1 and (not _soak_stats["intervals"] or 
        all(i["error_rate"] < 2 for i in _soak_stats["intervals"])):
        print("\n✅ SOAK TEST PASSED")
