...
class TestRedisPartition:
    ...
    def test_redis_timeout_product_list(self, toxiproxy: ToxiproxyClient, app_url: str):
    ...
    def test_redis_latency_degrades_gracefully(self, toxiproxy: ToxiproxyClient, app_url: str):
...
class TestPostgresPartition:
    ...
    def test_db_down_cached_data_available(self, toxiproxy: ToxiproxyClient, app_url: str):
        """DB 다운 시 캐시된 데이터 조회"""
        # 1. 먼저 정상 요청으로 캐시 워밍업
        resp = make_request(app_url, "/api/products/")
        assert resp.status_code == 200
    ...
    def test_db_latency_query_timeout(self, toxiproxy: ToxiproxyClient, app_url: str):
        """DB 지연 시 쿼리 타임아웃"""
        # DB에 5초 지연 추가
...
class TestPartialPartition:
    ...
    def test_redis_down_db_up_is_partial(self, toxiproxy: ToxiproxyClient, app_url: str):
    ...
    def test_recovery_after_partition(self, toxiproxy: ToxiproxyClient, app_url: str):
        """장애 복구 후 정상 동작"""
        # 1. 장애 주입
        toxiproxy.simulate_redis_down()
        time.sleep(1)
...
class TestGradualDegradation:
    ...
    def test_increasing_latency(self, toxiproxy: ToxiproxyClient, app_url: str):
        """점진적으로 증가하는 지연"""
        ...
        for latency in latencies:
            ...
            try:
                resp = make_request(app_url, "/api/products/", timeout=10)
                elapsed = time.time() - start
                results.append({"latency": latency, "status": resp.status_code, "elapsed": elapsed})
            except requests.exceptions.Timeout:
                results.append({"latency": latency, "status": "timeout", "elapsed": 10})
...
class TestFlappingConnection:
    ...
    def test_intermittent_failures(self, toxiproxy: ToxiproxyClient, app_url: str):
        """50% 확률로 연결 리셋"""
        # 50% 확률로 연결 리셋
        toxiproxy.add_toxic("redis", ToxicType.RESET_PEER, {"timeout": 0}, toxicity=0.5)  # 50% 확률
        time.sleep(0.5)
...
class TestToxiproxySetup:
    ...
    def test_can_add_and_remove_toxic(self, toxiproxy: ToxiproxyClient):
        ...
        # Toxic 추가
        toxic = toxiproxy.add_toxic(proxy_name, ToxicType.LATENCY, {"latency": 100}, name="test_toxic")
