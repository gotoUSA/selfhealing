...
⚠️  전역 tests 폴더는 통합 테스트 전용입니다.
    Unit Test는 packages/selfhealing-python/tests/unit/ 에서 진행하세요.
    ...
    def run_chain(self, *tasks) -> Any:
        ...
        for task_info in tasks:
            if isinstance(task_info, str):
                ...
                name, args, kwargs = task_info[0], task_info[1] if len(task_info) > 1 else (), task_info[2] if len(task_info) > 2 else {}
