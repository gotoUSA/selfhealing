                ...
                if response.status_code == 503:
                    ...
                    if _extreme_stats["circuit_breaker"]["open_detected"]:
                        if not _extreme_stats["circuit_breaker"]["recovery_detected"]:
                            ...
                            _extreme_stats["circuit_breaker"]["state_history"].append({"state": "CLOSED", "time": time.time()})
...
def on_test_stop(environment, **kwargs):
    ...
    pool_exhausted = (
        _extreme_stats["pool_exhausted_count"] > 0 or _connection_failures["count"] >= _connection_failures["threshold"]
    )
...
class PoolKillerUser(PatchedHttpUser):
    ...
    def hold_connection_long(self):
        ...
        endpoint = random.choice(["/api/self-healing/stress/slow-5s/", "/api/self-healing/stress/slow-10s/"])

        with self.client.get(endpoint, catch_response=True, name=f"{STAGE_NAME} Connection Holder", timeout=15) as response:
            elapsed = (time.time() - start) * 1000
...
class MonitorUser(PatchedHttpUser):
    ...
    def health_check(self):
        ...
        with self.client.get(
            "/api/self-healing/health/", catch_response=True, name=f"{STAGE_NAME} Health Check", timeout=15  # 2분 대기
        ) as response:
            elapsed = (time.time() - start) * 1000
...
class NormalUser(PatchedHttpUser):
    ...
    def category_list(self):
        ...
        with self.client.get(
            "/api/categories/", catch_response=True, name=f"{STAGE_NAME} Normal - Categories", timeout=15  # 2분 대기
        ) as response:
            elapsed = (time.time() - start) * 1000
